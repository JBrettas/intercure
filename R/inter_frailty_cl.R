...
gera_ksih_effect <- function(y_h, data_set, delta, cov_theta, cov_beta,
                             grupo, theta, beta, gamma, nelson_aalen_function){
  ...
  for (i in 1:m) {
    ...
    b_h[i] <- exp(gamma) +
      sum((exp(theta %*%
                 t(cbind(1,data_set[grupo == unique(grupo)[i],
                                    cov_theta]))) / 2) *
            (1 - 1 / (2 * exp(beta %*%
                                t(data_set[grupo == unique(grupo)[i],
                                           cov_beta])) *
                        (sapply(y_h[grupo == unique(grupo)[i]],
...
gera_kh_effect <- function(y_h, ksi_h, data_set, delta,
                           cov_theta, cov_beta, grupo, theta, beta,
...
f_cond_effect <- function(tempo, l, r,
                          ...
                          cov_theta, cov_beta, nelson_aalen_function, dataset){
  s_cl <- function(t_gen) surv_cl(t_gen, i, j,
                                  theta, beta, gamma, cov_theta, cov_beta,
                                  grupo, nelson_aalen_function, dataset)
        ...
        times_cl_i[j] <- l_cl_i[j]
         ...
         if (l_cl_i[j] == r_cl_i[j]) {
          ...
          cumf_y_cond <- function(x) f_cond_effect(x, l_cl_i[j], r_cl_i[j],
                                                   ...
                                                   nelson_aalen_function,
                                                   dataset)
          inverse_f_cond_effect <- inverse(function(x) cumf_y_cond(x),
                                           l_cl_i[j], r_cl_i[j])
...
inter_frailty_cl <- function(dataset, left, right, delta, cov_theta, cov_beta,
                             grp, M, b=0.001, max_n=100, par_cl = NULL,
  ...
  while(!conv | n <= burn_in) {
    ...
    if(!is.null(par_cl)){
      list_reg <- foreach(iterators::icount(M),
                          ...
                          .export=c("surv_lam","inverse_lam_f","surv_cl",
                                    "inverse", "f_cond_effect",
                                    "gera_yh_effect",
      ...
      list_reg <- foreach(iterators::icount(M),
                          .packages=c("MASS","MLEcens","Matrix","survival",
                                      "stats4","plyr"),
                          .export=c("surv_lam","inverse_lam_f",
                                    "surv_cl","inverse",
    ...
    SUM_VAR <- as.matrix(bdiag(list(sum_var_theta,
                                    sum_var_beta,
                                    sum_var_gamma)))
