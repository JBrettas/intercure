...
gera_ksih_effect <- function(y_h, data_set, delta, cov_theta, cov_beta, grupo, theta, beta, gamma, nelson_aalen_function){
  m <- length(unique(grupo))
  a_h = b_h = rep(NA, m)
  for(i in 1:m) {
    a_h[i] <- exp(gamma) + sum(delta[(grupo == unique(grupo)[i])])
    b_h[i] <- exp(gamma) + sum((exp(theta %*% t(cbind(1,data_set[grupo == unique(grupo)[i], cov_theta]))) / 2) *
                                 (1 - 1 / (2 * exp(beta %*% t(data_set[grupo == unique(grupo)[i], cov_beta])) *
                                             (sapply(y_h[grupo == unique(grupo)[i]], nelson_aalen_function)) + 1)))
  }
  rgamma(m, a_h, b_h)
}

# Generates a vector of n latent variables k
gera_kh_effect <- function(y_h, ksi_h, data_set, delta, cov_theta, cov_beta, grupo, theta, beta, nelson_aalen_function){
  ...
  den <- 2 + 4 * sapply(y_h, nelson_aalen_function) * exp(as.vector(beta %*% t(xi_1)))
...
surv_cl <- function(tempo, i, j, theta, beta, gamma, cov_theta, cov_beta, grupo, nelson_aalen_function, dataset) {
  w <- exp(gamma)
  ...
  if (j == 1) {
    surv_1_cl_i <- (1 + (etas_i[1] / 2 * w) * (1 - 1 / (2 * mus_i[1] * nelson_aalen_function(tempo) + 1))) ^ (-w)
    return(surv_1_cl_i)
  }
  num <- (etas_i[j] / 2) * (1 - (1 / (2 * mus_i[j] * nelson_aalen_function(tempo) + 1)))
  den <- w + sum((etas_i[1:(j-1)] / 2) * (1 - (1 / (2 * mus_i[1:(j-1)] * nelson_aalen_function(tempo) + 1))))
  surv_j_cl_i <- (1 + num / den)^(-w - sum(base_cl$delta[1:(j - 1)]))
  return(surv_j_cl_i)
}

f_cond_effect <- function(tempo, l, r, i, j, theta, beta, gamma, cov_theta, cov_beta, nelson_aalen_function, dataset){
  s_cl <- function(t_gen) surv_cl(t_gen, i, j, theta, beta, gamma, cov_theta, cov_beta, grupo, nelson_aalen_function, dataset)
  naalen_mod <- function(t_gen) {
    (nelson_aalen_function(l) * (r - t_gen) + nelson_aalen_function(r) * (t_gen - l)) / (r - l)
  }
  s_cl_mod <- function(t_gen) surv_cl(t_gen, i, j, theta, beta, gamma, cov_theta, cov_beta, grupo, naalen_mod, dataset)
...
gera_yh_effect <- function(left, right, delta, cov_theta, cov_beta, grupo, theta, beta, gamma, nelson_aalen_function, dataset){
  new_yh <- NA
  for(i in unique(grupo)) {
    times_cl_i <- rep(NA,nrow(dataset[grupo == i,]))
    delta_cl_i <- delta[grupo == i]
    l_cl_i <- left[grupo == i]
    r_cl_i <- right[grupo == i]
      ...
      if(delta_cl_i[j] == 0) {
        times_cl_i[j] = l_cl_i[j]
         ...
         if (l_cl_i[j] == r_cl_i[j]) {
          ...
          #print(j)
          u_var <- runif(1)
          cumf_y_cond <- function(x) f_cond_effect(x, l_cl_i[j], r_cl_i[j], i, j, theta, beta, gamma, cov_theta, cov_beta, nelson_aalen_function, dataset)
          inverse_f_cond_effect <- inverse(function(x) cumf_y_cond(x), l_cl_i[j], r_cl_i[j])
...
#' and a dummy variable \code{stop_c} assuming 0 if algorithm converged and 1 if
...
#' @export
#' @import foreach
inter_frailty_cl <- function(data_set, left, right, delta, cov_theta, cov_beta, grp, M, b=0.001, n_int_max=100, par_cl = NULL, burn_in=50, outputFiles = FALSE) {
  ...
  y_n_M = k_n_M = u_n_M = ksi_n_M = matrix(NA, nrow = M, ncol = nrow(data_set))
  ...
  Vetores_NAalen <- nelson_aalen_table(data_set, y_n_M[1,], delta, beta, cov_beta, u)
  naalen_avg<- stepfun(Vetores_NAalen$time, c(0,Vetores_NAalen$hazard))
  ...
  while(!conv | n <= burn_in) {
    ...
    if(!is.null(par_cl)){
      list_reg <- foreach(iterators::icount(M), .packages=c("MASS","MLEcens","Matrix","survival","stats4","plyr"), .export=c("surv_lam","inverse_lam_f","surv_cl","inverse","f_cond_effect", "gera_yh_effect","gera_ksih_effect", "gera_kh_effect","log_vero_gamma", "gera_uh"), .inorder=F) %dopar% {
        ...
        y <- as.numeric(gera_yh_effect(left, right, delta, cov_theta, cov_beta, grp, theta_M, beta_M, gamma_M, naalen_avg, data_set))
        ksi <- gera_ksih_effect(y, data_set, delta, cov_theta, cov_beta, grp, theta_M, beta_M, gamma_M, naalen_avg)
        k_aux <- gera_kh_effect(y, ksi, data_set, delta, cov_theta, cov_beta, grp, theta_M, beta_M, naalen_avg)
        k <- k_aux$k
        ksi_geral <- k_aux$ksi
        u <- gera_uh(y, k, data_set, right, delta, cov_beta, beta_M, naalen_avg)
      ...
      list_reg <- foreach(iterators::icount(M), .packages=c("MASS","MLEcens","Matrix","survival","stats4","plyr"), .export=c("surv_lam","inverse_lam_f","surv_cl","inverse","f_cond_effect", "gera_yh_effect","gera_ksih_effect", "gera_kh_effect","log_vero_gamma", "gera_uh"), .inorder=F) %do% {
        a_M <- MASS::mvrnorm(n=1, alpha, sigma_alpha)
        theta_M <- a_M[1:compr_theta]
        beta_M <- a_M[(compr_theta + 1):(compr_alpha - 1)]
        gamma_M <- a_M[compr_alpha]
    ...
    if(!is.null(par_cl)){
      step_list <- foreach::foreach(h=1:M, .export="nelson_aalen_table", .inorder=F) %dopar% {
        V_NAalen <- nelson_aalen_table(data_set, y_n_M[h,], delta, beta_M[h,], cov_beta, u_n_M[h,])
    ...
    #V_NAalen<-aux_naalen(sort(y_n_M[,delta==1]),naalen_avg,cl)
    V_NAalen <- aux_naalen(sort(y_n_M[,delta==1]), naalen_avg, par_cl)
    naalen_avg_new <- stepfun(V_NAalen$time, c(0, V_NAalen$hazard))
